    ...
    <div className="bg-brand-card border border-brand-border rounded-xl p-5 hover:border-brand-muted transition-colors">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0">
          <h3 className="font-display font-bold text-brand-white truncate">{client.companyName}</h3>
          {client.contactName && (
            <p className="text-brand-muted text-sm truncate">{client.contactName}</p>
          )}
        </div>
        <div className="flex gap-1 shrink-0">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-brand-muted hover:text-red-400 hover:bg-brand-card2 transition-colors"
          >
            <Trash2 size={15} />
          </button>
        </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-brand-border">
        ...
        {total > 0 && (
          <span className="font-mono text-xs text-lime">{formatCurrency(total)}</span>
        )}
      </div>
    </div>
  );
